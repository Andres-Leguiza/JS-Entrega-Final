...
.then((data) => {
    ...
    btnOrderByPriceAsc.addEventListener("click", () => agregarEstadoActive(btnDropPrecio))
    let btnOrderByPriceDesc = document.getElementById("orderBy-PriceDesc");
    btnOrderByPriceDesc.addEventListener("click", () => ordenarPor("precioDesc", destinos));
    btnOrderByPriceDesc.addEventListener("click", () => agregarEstadoActive(btnDropPrecio))
    let btnOrderByCountry = document.getElementById("orderBy-Country");
    btnOrderByCountry.addEventListener("click", () => ordenarPor("pais", destinos));
    btnOrderByCountry.addEventListener("click", () => agregarEstadoActive(btnOrderByCountry))
    let btnOrderByCity = document.getElementById("orderBy-City");
    btnOrderByCity.addEventListener("click", () => ordenarPor("ciudad", destinos));
    btnOrderByCity.addEventListener("click", () => agregarEstadoActive(btnOrderByCity))
...
function ordenarPor(ordenarPor, destinos){
    let section = document.getElementById("grid-destinos")
    switch (ordenarPor){
        case "precioAsc":
            let ordenadoPorPrecioAsc = destinos.sort((a, b) => a.precio - b.precio)
            section.innerHTML = ""
            mostrarDestinos(ordenadoPorPrecioAsc)
            break;
        case "precioDesc":
            let ordenadoPorPrecioDesc = destinos.sort((a, b) => b.precio - a.precio)
            section.innerHTML = ""
            mostrarDestinos(ordenadoPorPrecioDesc)
            break;
        case "pais":
            let ordenadoPorPais = destinos.sort((a, b) => a.paisDestino.localeCompare(b.paisDestino))
            section.innerHTML = ""
            mostrarDestinos(ordenadoPorPais)
            break;
        case "ciudad":
            let ordenadoPorCiudad = destinos.sort((a, b) => a.ciudadDestino.localeCompare( b.ciudadDestino))
            section.innerHTML = ""
...
function agregarEstadoActive(elementoActive){
    ...
    [...orderBy.children].forEach(el => el.classList.remove("active"))
...
function calcularPrecio(cantidad, precio){
    if(cantidad >= 2){
        ...
        return `$${cantidad*precio*0.75} <s style="color: #6c757d">($${cantidad*precio})</s>`
    } else {
